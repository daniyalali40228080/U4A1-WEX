...
int studentLoginAttempts = 3; // Number of password attempts allowed

void studentLogin() {

	while (studentLoginAttempts > 0) {
		...
		if (studentUsername == "student" && studentPassword == "student") {
			...
			cout << endl << "You have " << studentLoginAttempts << " login attempts remaining." << endl;
			studentLogin();
		}
	}
